...
__global__ void addKernel(int* c, const int* a, const int* b)
{
    ...
    //fixed the global index fuction should work now since i used blocks instead of threads
    //this should work for 2D arrays now since i am using threads for x and y
    int i = threadIdx.y * blockDim.x + threadIdx.x;
   if (i<arraySize) c[i] = a[i] + b[i];
}
...
//this is now lab2 we're still using the old code from lab1 but it seems like instead of using one block for variables like threadIdx.x and blockIdx.x were doing 2D arrays for that?
int main()
{
    ...
    // 3. DEVICE: Compute
    // Ok so now it should work when adding the input vectors and printing to kernel added in the size variable thing ok it now works but i need to delete the pdb file in x64 each time I reopen the project this is stupid
    //should be working now since i used a different solution
    int size = 5;
